                ...
                <Popover key={index}>
                  <PopoverTrigger asChild>
                    <div className='relative group'>
                      <p
                        className={twMerge(
                          "no-underline  uppercase hover:text-white transition-all text-sm cursor-pointer bg-gradient-to-r from-emerald-300 to-sky-400 bg-clip-text text-transparent font-semibold",
                        )}>
                        {nav.title}
                      </p>
            ...
            if (index === 4) {
              return (
                <Popover key={index}>
                  <PopoverTrigger asChild>
                    <div className='relative group'>
                      <p className={twMerge("no-underline uppercase hover:text-white transition-all text-sm cursor-pointer")}>{nav.title}</p>
                      <div className='h-[1px] w-full bg-white/80 absolute bottom-0 left-0 origin-left scale-x-0 transition-transform duration-300 group-hover:scale-x-100'></div>
                    </div>
                  </PopoverTrigger>
                  <PopoverContent className='w-80 rounded-2xl border border-neutral-200/70 bg-white p-3 shadow-[0_10px_40px_-18px_rgba(0,0,0,0.25)]'>
            ...
            return (
              <div key={index} className='relative group'>
                <Link
                  href={nav.herf}
                  className={twMerge(
                    "no-underline text-white/80 uppercase hover:text-white transition-all text-sm",
